...
获取城市当前的天气
"""
class WeatherNowMCPTool(BaseTool):
...
获取城市未来几天的天气
"""
class WeatherFutherMCPTool(BaseTool):
    """Weather MCP tool using Seniverse (心知天气) API."""
    ...
    def get_description(self) -> str:
        return "天气查询MCP工具：根据城市名称查询未来几天的天气（基于心知天气API）。"
    ...
    async def execute(self, **kwargs) -> ToolResult:
        ...
        try:
            ...
            # Ensure location is a str and preserve unicode
            location_val = params.get("location")
            days = 5 if not params.get("days") else params.get("days")
            if isinstance(location_val, bytes):
                try:
                    location_val = location_val.decode("utf-8")
                except Exception:
